...
local PANEL, PART, HUB = 5001, 5002, 999
local AID
local TYPEMAP, IIDTYPE, STATE = {}, {}, {}
local USER_CODE = ""

local HK = { CUR = "66", TGT = "67", ALARMTYPE = "8E", FAULT = "77", TAMPER = "7A" }

local ARM_TO_HK = { Home = 0, Away = 1, Night = 2, Vacation = 1, ["Custom Bypass"] = 0 }
local HK_TO_ARM = { [0] = "Home", [1] = "Away", [2] = "Night" }
...
local function notify(b, cmd, p) C4:SendToProxy(b, cmd, p or {}, "NOTIFY") end
...
local LAST_STATE, LAST_TYPE
local function update_partition(state, atype)
  if state == LAST_STATE and atype == LAST_TYPE then return end
  LAST_STATE, LAST_TYPE = state, atype
  notify(PART, "PARTITION_STATE", { STATE = state, TYPE = atype, CODE_REQUIRED_TO_CLEAR = false })
  dbg("partition -> " .. state .. (atype ~= "" and (" (" .. atype .. ")") or ""))
end

local function reflect()
  local cur = tonumber(STATE[HK.CUR])
  local tgt = tonumber(STATE[HK.TGT])
  if cur == nil then return end
  if cur == 4 then
    update_partition("ALARM", "Burglary")
  elseif cur == 3 then
    if tgt ~= nil and tgt ~= 3 then update_partition("EXIT_DELAY", HK_TO_ARM[tgt] or "")
    else update_partition("DISARMED_READY", "") end
  else
    update_partition("ARMED", HK_TO_ARM[cur] or "")
  end
end
...
local function apply(list)
  ...
  reflect()
  local tamper = STATE[HK.TAMPER]
  if tamper == true or tonumber(tamper) == 1 then dbg("TAMPER detected") end
end

local function announce_code_policy()
  notify(PART, "CODE_REQUIRED", { CODE_REQUIRED_TO_ARM = (USER_CODE ~= "") })
end

function DRV.OnDriverLateInit()
  AID = aidfmt(Properties and Properties["Accessory AID"] or "1")
  USER_CODE = tostring((Properties and Properties["User Code"]) or "")
  dbg("alarm init, aid=" .. tostring(AID) .. " code=" .. (USER_CODE ~= "" and "required" or "not required"))
  notify(PART, "PARTITION_ENABLED", { ENABLED = "true" })
  notify(PART, "PARTITION_STATE_INIT", { STATE = "DISARMED_NOT_READY", CODE_REQUIRED_TO_CLEAR = false })
  notify(PANEL, "PANEL_INITIALIZED", {})
  announce_code_policy()
  to_hub("HK_GET_STATE")
end
OPC["Accessory AID"] = function(v) AID = aidfmt(v); LAST_STATE = nil; to_hub("HK_GET_STATE") end
OPC["User Code"] = function(v) USER_CODE = tostring(v or ""); announce_code_policy() end
...
function RFP.RECEIVE_DB(idBinding, tParams)
  ...
  if ok then
    set_iid_maps(map)
    dbg("got DB: TargetState iid=" .. tostring(TYPEMAP[HK.TGT]) ..
        " CurrentState iid=" .. tostring(TYPEMAP[HK.CUR]))
    reflect()
  end
end
...
local function code_ok(supplied)
  if USER_CODE == "" then return true end
  return tostring(supplied or "") == USER_CODE
end

local function set_target(v)
  local iid = TYPEMAP[HK.TGT]
  if not iid then dbg("no SecuritySystemTargetState iid (not bound / wrong aid?)"); return end
  to_hub("HK_SET_CHAR", { iid = tostring(iid), value = tostring(v) })
end

function RFP.PARTITION_ARM(idBinding, tParams)
  local armType = tParams.ArmType or "Away"
  if not code_ok(tParams.UserCode) then
    dbg("ARM refused: wrong user code"); reflect(); return
  end
  local hk = ARM_TO_HK[armType]
  if hk == nil then dbg("unknown ArmType '" .. tostring(armType) .. "', using Away"); hk = 1 end
  dbg("proxy PARTITION_ARM " .. armType .. " -> HK target " .. hk)
  set_target(hk)
end

function RFP.PARTITION_DISARM(idBinding, tParams)
  if not code_ok(tParams.UserCode) then
    dbg("DISARM refused: wrong user code"); reflect(); return
  end
  dbg("proxy PARTITION_DISARM -> HK target 3")
  set_target(3)
end

function RFP.KEY_PRESS(idBinding, tParams)
  dbg("KEY_PRESS " .. tostring(tParams and tParams.KeyName) .. " (no HomeKit equivalent)")
end
function RFP.EXECUTE_EMERGENCY(idBinding, tParams)
  dbg("EXECUTE_EMERGENCY " .. tostring(tParams and tParams.EmergencyType) .. " (no HomeKit equivalent)")
end
