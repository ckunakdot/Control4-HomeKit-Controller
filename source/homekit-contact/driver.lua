...
local CONTACT, HUB = 1, 999
local AID
local TYPEMAP, IIDTYPE, STATE = {}, {}, {}
local INVERT = false
local WANT = "Auto"
local SERVICES, WANT_ZONE = {}, "Auto"

local SENSORS = {
  { t = "6A", name = "Contact",   active = function(v) return tonumber(v) == 1 end },
  { t = "22", name = "Motion",    active = function(v) return v == true or tonumber(v) == 1 end },
  { t = "71", name = "Occupancy", active = function(v) return tonumber(v) == 1 end },
  { t = "76", name = "Smoke",     active = function(v) return tonumber(v) == 1 end },
  { t = "70", name = "Leak",      active = function(v) return tonumber(v) == 1 end },
}
local BATT, LOWBATT, TAMPER = "68", "79", "7A"
...
local function active_sensor()
  if WANT_ZONE ~= "Auto" and #SERVICES > 0 then
    for _, sv in ipairs(SERVICES) do
      if sv.name == WANT_ZONE then
        for _, sp in ipairs(SENSORS) do
          if (WANT == "Auto" or sp.name == WANT) and sv.chars and sv.chars[sp.t] then
            return sp, sv.chars[sp.t]
          end
        end
      end
    end
  ...
  if WANT ~= "Auto" then
    for _, s in ipairs(SENSORS) do
      if s.name == WANT then
        if TYPEMAP[s.t] then return s, TYPEMAP[s.t] end
        return nil, nil
      end
    end
    return nil, nil
  end
  for _, s in ipairs(SENSORS) do if TYPEMAP[s.t] then return s, TYPEMAP[s.t] end end
  return nil, nil
end

local function available_sensors()
  local t = {}
  for _, s in ipairs(SENSORS) do if TYPEMAP[s.t] then t[#t + 1] = s.name end end
  return table.concat(t, ", ")
end

local LAST
local function reflect()
  local s, iid = active_sensor(); if not (s and iid) then return end
  local v = STATE[tonumber(iid) or iid]; if v == nil then return end
  local on = s.active(v)
  if INVERT then on = not on end
  if on ~= LAST then
    LAST = on
    notify(CONTACT, on and "OPENED" or "CLOSED")
    dbg(s.name .. (WANT_ZONE ~= "Auto" and (" [" .. WANT_ZONE .. "]") or "")
        .. " -> " .. (on and "OPENED" or "CLOSED"))
  end
end

local function apply(list)
  for _, c in ipairs(list) do
    STATE[tonumber(c.iid) or c.iid] = c.value
  end
  reflect()
end
...
function DRV.OnDriverLateInit()
  ...
  INVERT = (Properties and Properties["Invert"] == "Yes")
  WANT = (Properties and Properties["Sensor Type"]) or "Auto"
  WANT_ZONE = (Properties and Properties["Zone"]) or "Auto"
  dbg("contact init, aid=" .. tostring(AID) .. " sensor=" .. WANT .. " zone=" .. WANT_ZONE)
  to_hub("HK_GET_STATE")
end
OPC["Accessory AID"] = function(v) AID = aidfmt(v); LAST = nil; to_hub("HK_GET_STATE") end
OPC["Invert"] = function(v) INVERT = (v == "Yes"); LAST = nil; reflect() end
OPC["Zone"] = function(v)
  WANT_ZONE = v or "Auto"; LAST = nil
  dbg("zone -> " .. WANT_ZONE)
  to_hub("HK_GET_STATE")
end
OPC["Sensor Type"] = function(v)
  WANT = v or "Auto"; LAST = nil
  dbg("sensor type -> " .. WANT)
  to_hub("HK_GET_STATE")
end
...
function RFP.RECEIVE_DB(idBinding, tParams)
  ...
  if ok then
    set_iid_maps(map)
    local s = active_sensor()
    local have = available_sensors()
    if s then
      dbg("got DB: using " .. s.name .. " (iid " .. tostring(TYPEMAP[s.t]) .. "); accessory offers: " .. have)
    else
      dbg("got DB: NO usable sensor. Sensor Type='" .. WANT .. "' but this accessory offers: "
          .. (have ~= "" and have or "none"))
    end
    reflect()
  end
end
...
function RFP.RECEIVE_SERVICES(idBinding, tParams)
  ...
  for _, sv in ipairs(SERVICES) do
    if sv.name and not seen[sv.name] then seen[sv.name] = true; names[#names + 1] = (sv.name:gsub(",", " ")) end
  end
  pcall(function() C4:UpdatePropertyList("Zone", table.concat(names, ",")) end)
  dbg("services: " .. (#SERVICES) .. "; zones offered: " .. table.concat(names, ", "))
  LAST = nil
  reflect()
end
 end)
