...
local RELAY, CLOSED_S, OPEN_S, HUB = 1, 2, 3, 999
local AID
local TYPEMAP, IIDTYPE, STATE = {}, {}, {}
local HK = { CUR = "E", TGT = "32", OBSTRUCT = "24" }
...
local function reflect()
  ...
  LAST = cur
  if cur == 1 then
    notify(CLOSED_S, "CLOSED"); notify(OPEN_S, "OPENED"); dbg("door CLOSED")
  elseif cur == 0 then
    notify(CLOSED_S, "OPENED"); notify(OPEN_S, "CLOSED"); dbg("door OPEN")
  else
    notify(CLOSED_S, "OPENED"); notify(OPEN_S, "OPENED")
    dbg("door in transit (state " .. cur .. ")")
  end
end
...
local function apply(list)
  ...
  reflect()
  local ob = STATE[HK.OBSTRUCT]
  if ob == true or tonumber(ob) == 1 then dbg("OBSTRUCTION DETECTED") end
end

function DRV.OnDriverLateInit()
  AID = aidfmt(Properties and Properties["Accessory AID"] or "1")
  dbg("garage init, aid=" .. tostring(AID))
  to_hub("HK_GET_STATE")
end
...
function RFP.RECEIVE_DB(idBinding, tParams)
  ...
  if ok then
    set_iid_maps(map)
    dbg("got DB: TargetDoorState iid=" .. tostring(TYPEMAP[HK.TGT]) ..
        " CurrentDoorState iid=" .. tostring(TYPEMAP[HK.CUR]))
    reflect()
  end
end
...
local function set_target(v)
  ...
  if iid then to_hub("HK_SET_CHAR", { iid = tostring(iid), value = tostring(v) })
  else dbg("no TargetDoorState iid (not bound / wrong aid?)") end
end

function RFP.OPEN()  dbg("relay OPEN");  set_target(0) end
function RFP.CLOSE() dbg("relay CLOSE"); set_target(1) end
function RFP.TOGGLE()
  local cur = tonumber(STATE[HK.CUR]) or 1
  dbg("relay TOGGLE (current " .. cur .. ")")
  if cur == 0 or cur == 2 then set_target(1) else set_target(0) end
end
