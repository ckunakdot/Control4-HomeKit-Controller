...
local AID
local TYPEMAP, IIDTYPE, STATE = {}, {}, {}
local HK = { ON = "25", BRIGHT = "8", HUE = "13", SAT = "2F", CT = "CE" }
...
local function clampi(n) n = tonumber(n) or 0; if n < 0 then return 0 elseif n > 100 then return 100 end return math.floor(n + 0.5) end

local function gamma_e(c) if c <= 0.0031308 then return 12.92 * c else return 1.055 * c ^ (1/2.4) - 0.055 end end
local function gamma_d(c) if c <= 0.04045 then return c / 12.92 else return ((c + 0.055) / 1.055) ^ 2.4 end end

local function xy_to_hs(x, y)
  x = tonumber(x); y = tonumber(y)
  if not x or not y or y == 0 then return nil end
  local Y = 1.0
  local X = (x / y) * Y
  local Z = ((1 - x - y) / y) * Y
  ...
  local m = math.max(r, g, b); if m > 0 then r, g, b = r/m, g/m, b/m end
  r, g, b = gamma_e(r), gamma_e(g), gamma_e(b)
...
local function hs_to_xy(h, s)
  h = (tonumber(h) or 0) % 360; s = (tonumber(s) or 0) / 100
  local v, c = 1, 1 * s
  local xx = c * (1 - math.abs((h / 60) % 2 - 1)); local mm = v - c
  local r, g, b
  if h < 60 then r, g, b = c, xx, 0 elseif h < 120 then r, g, b = xx, c, 0
  elseif h < 180 then r, g, b = 0, c, xx elseif h < 240 then r, g, b = 0, xx, c
  elseif h < 300 then r, g, b = xx, 0, c else r, g, b = c, 0, xx end
  r, g, b = gamma_d(r + mm), gamma_d(g + mm), gamma_d(b + mm)
...
local function mired_to_k(m) m = tonumber(m); return m and m > 0 and math.floor(1e6 / m + 0.5) or nil end
local function k_to_mired(k) k = tonumber(k); return k and k > 0 and math.floor(1e6 / k + 0.5) or nil end

function DRV.OnDriverLateInit()
  AID = aidfmt(Properties and Properties["Accessory AID"] or "1")
  dbg("light init, aid=" .. tostring(AID)); to_hub("HK_GET_STATE")
end
...
local function mark_online()
  if not ONLINE then notify("ONLINE_CHANGED", { STATE = true }); ONLINE = true; dbg("light -> ONLINE") end
end

local function current_level()
  local on = STATE[HK.ON]
  local onb = (on == true or on == 1 or on == "1" or on == "true")
  if TYPEMAP[HK.BRIGHT] then
    local b = tonumber(STATE[HK.BRIGHT])
    if b ~= nil then return onb and clampi(b) or 0 end
  end
  return onb and 100 or 0
end

local function reflect()
  notify("LIGHT_BRIGHTNESS_CHANGED", { LIGHT_BRIGHTNESS_CURRENT = current_level() })
  if TYPEMAP[HK.HUE] and STATE[HK.HUE] ~= nil and STATE[HK.SAT] ~= nil then
    local x, y = hs_to_xy(STATE[HK.HUE], STATE[HK.SAT])
    if x then notify("LIGHT_COLOR_CHANGED", { LIGHT_COLOR_CURRENT_X = x, LIGHT_COLOR_CURRENT_Y = y, LIGHT_COLOR_CURRENT_MODE = 0 }) end
  elseif TYPEMAP[HK.CT] and STATE[HK.CT] ~= nil then
    local k = mired_to_k(STATE[HK.CT])
    if k then notify("LIGHT_COLOR_CHANGED", { LIGHT_COLOR_CURRENT_COLOR_TEMPERATURE = k, LIGHT_COLOR_CURRENT_MODE = 1 }) end
  end
end

local CAPS_SENT = false
local function send_caps()
  if CAPS_SENT then return end
  local hasColor = (TYPEMAP[HK.HUE] and TYPEMAP[HK.SAT]) and true or false
  local hasCCT = TYPEMAP[HK.CT] and true or false
  notify("DYNAMIC_CAPABILITIES_CHANGED", {
    dimmer = TYPEMAP[HK.BRIGHT] ~= nil,
    set_level = TYPEMAP[HK.BRIGHT] ~= nil,
    supports_target = TYPEMAP[HK.BRIGHT] ~= nil,
    supports_color = hasColor,
    supports_color_correlated_temperature = hasCCT,
    color_correlated_temperature_min = hasCCT and 2200 or 0,
    color_correlated_temperature_max = hasCCT and 6500 or 0,
  })
  CAPS_SENT = true
  dbg("caps: color=" .. tostring(hasColor) .. " cct=" .. tostring(hasCCT))
end
...
function RFP.RECEIVE_DB(idBinding, tParams)
  ...
  if ok then set_iid_maps(map)
    dbg("got DB: On iid=" .. tostring(TYPEMAP[HK.ON]) .. " Brightness iid=" .. tostring(TYPEMAP[HK.BRIGHT])
        .. " Hue iid=" .. tostring(TYPEMAP[HK.HUE]) .. " CT iid=" .. tostring(TYPEMAP[HK.CT]))
    if TYPEMAP[HK.ON] or TYPEMAP[HK.BRIGHT] then mark_online(); send_caps(); reflect() end
  end
end
...
local function set_on(b)
  local iid = TYPEMAP[HK.ON]
  if iid then to_hub("HK_SET_CHAR", { iid = tostring(iid), value = b and "true" or "false" }) end
end
local function set_brightness(level)
  level = clampi(level)
  if level == 0 then set_on(false); return end
  local biid = TYPEMAP[HK.BRIGHT]
  set_on(true)
  if biid then to_hub("HK_SET_CHAR", { iid = tostring(biid), value = tostring(level) }) end
end

local function write_char(hk_type, value)
  local iid = TYPEMAP[hk_type]
  if iid then to_hub("HK_SET_CHAR", { iid = tostring(iid), value = tostring(value) }) end
end
local function write_chars(chars) to_hub("HK_SET_CHARS", { data = JSON:encode(chars) }) end

function RFP.SET_COLOR_TARGET(idBinding, tParams)
  local x, y = tParams.LIGHT_COLOR_TARGET_X, tParams.LIGHT_COLOR_TARGET_Y
  local ct = tParams.LIGHT_COLOR_TARGET_COLOR_TEMPERATURE
  local mode = tonumber(tParams.LIGHT_COLOR_TARGET_MODE)
  if (mode == 1 or (ct and not x)) and TYPEMAP[HK.CT] then
    local mired = k_to_mired(ct)
    if mired then
      dbg("proxy SET_COLOR cct " .. tostring(ct) .. "K")
      write_chars({ { iid = TYPEMAP[HK.ON], value = true }, { iid = TYPEMAP[HK.CT], value = mired } })
    end
  elseif x and y and TYPEMAP[HK.HUE] and TYPEMAP[HK.SAT] then
    local h, s = xy_to_hs(x, y)
    if h then
      dbg(("proxy SET_COLOR xy(%s,%s) -> H%.0f S%.0f"):format(tostring(x), tostring(y), h, s))
      write_chars({ { iid = TYPEMAP[HK.ON], value = true },
                    { iid = TYPEMAP[HK.HUE], value = math.floor(h + 0.5) },
                    { iid = TYPEMAP[HK.SAT], value = clampi(s) } })
    end
  end
end

function RFP.UPDATE_COLOR_PRESET() end
function RFP.PUSH_COLOR_PRESET() end

function RFP.ON()  dbg("proxy ON");  set_on(true) end
function RFP.OFF() dbg("proxy OFF"); set_on(false) end

function RFP.SET_BRIGHTNESS_TARGET(idBinding, tParams)
  local lvl = tonumber(tParams.LIGHT_BRIGHTNESS_TARGET or tParams.LEVEL) or 0
  dbg("proxy SET_BRIGHTNESS " .. lvl); set_brightness(lvl)
end
function RFP.SET_LEVEL(idBinding, tParams)
  local lvl = tonumber(tParams.LEVEL or tParams.LIGHT_BRIGHTNESS_TARGET) or 0
  dbg("proxy SET_LEVEL " .. lvl); set_brightness(lvl)
end
RFP.GROUP_SET_LEVEL = RFP.SET_LEVEL
RFP.RAMP_TO_LEVEL = RFP.SET_LEVEL
RFP.GROUP_RAMP_TO_LEVEL = RFP.SET_LEVEL

function RFP.RECEIVE_META() end
 end)
