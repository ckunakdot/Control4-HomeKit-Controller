...
local HK = { CUR = "1D", TGT = "1E", BATT = "68", LOWBATT = "79" }
...
function DRV.OnDriverLateInit()
  ...
  dbg("lock init, aid=" .. tostring(AID)); to_hub("HK_GET_STATE")
end
...
local function lock_word(v)
  v = tonumber(v)
  if v == 1 then return "locked" elseif v == 0 then return "unlocked" else return "unknown" end
end

local function reflect()
  local cur = STATE[HK.CUR]
  if cur ~= nil then notify("LOCK_STATUS_CHANGED", { LOCK_STATUS = lock_word(cur) }) end
  local batt = tonumber(STATE[HK.BATT])
  if batt ~= nil then
    local s = (batt > 50) and "normal" or (batt > 25 and "warning" or "critical")
    notify("BATTERY_STATUS_CHANGED", { BATTERY_STATUS = s })
  end
end
...
function RFP.RECEIVE_DB(idBinding, tParams)
  ...
  if ok then set_iid_maps(map); dbg("got DB: LockTarget iid=" .. tostring(TYPEMAP[HK.TGT])) end
end
...
local function set_target(secured)
  local iid = TYPEMAP[HK.TGT]
  if iid then
    dbg("SET lock target=" .. tostring(secured) .. " -> iid " .. tostring(iid))
    to_hub("HK_SET_CHAR", { iid = tostring(iid), value = tostring(secured) })
  else
    dbg("no LockTargetState iid yet (map empty? not bound?)")
  end
end

function RFP.LOCK()   dbg("proxy LOCK");   set_target(1) end
function RFP.UNLOCK() dbg("proxy UNLOCK"); set_target(0) end
function RFP.TOGGLE()
  local cur = tonumber(STATE[HK.CUR]) or 0
  dbg("proxy TOGGLE (cur=" .. cur .. ")")
  set_target(cur == 1 and 0 or 1)
end
