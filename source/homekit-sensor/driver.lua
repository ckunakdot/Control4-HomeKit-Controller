...
local TEMP_B, HUM_B, HUB = 500, 600, 999
local AID
local TYPEMAP, IIDTYPE, STATE = {}, {}, {}
local HK = { TEMP = "11", HUM = "10", LUX = "6B" }
local SENT_T, SENT_H = false, false
local DARK_AT, BRIGHT_AT = 20, 30
local IS_DARK

local function read_thresholds()
  local d = tonumber(Properties and Properties["Dark Threshold (lux)"])
  local b = tonumber(Properties and Properties["Bright Threshold (lux)"])
  DARK_AT   = d or DARK_AT
  BRIGHT_AT = b or BRIGHT_AT
  if BRIGHT_AT <= DARK_AT then BRIGHT_AT = DARK_AT + 1 end
end
...
local function reflect()
  local t = tonumber(STATE[HK.TEMP])
  if t ~= nil then
    if not SENT_T then
      SENT_T = true
      notify(TEMP_B, "VALUE_INITIALIZE", { STATUS = "active", TIMESTAMP = tostring(os.time()) })
    end
    notify(TEMP_B, "VALUE_CHANGED", {
      CELSIUS = r1(t), FAHRENHEIT = r1(c2f(t)), TIMESTAMP = tostring(os.time()) })
  end
  local h = tonumber(STATE[HK.HUM])
  if h ~= nil then
    if not SENT_H then
      SENT_H = true
      notify(HUM_B, "VALUE_INITIALIZE", { STATUS = "active", TIMESTAMP = tostring(os.time()) })
    end
    notify(HUM_B, "VALUE_CHANGED", { VALUE = r1(h), TIMESTAMP = tostring(os.time()) })
  end
  local lux = tonumber(STATE[HK.LUX])

  local function setvar(n, v) pcall(function() C4:SetVariable(n, v) end) end
  local function setprop(n, v) pcall(function() C4:UpdateProperty(n, v) end) end
  local stamped = false
  if t ~= nil then
    setvar("TEMPERATURE_C", r1(t)); setvar("TEMPERATURE_F", r1(c2f(t)))
    setprop("Temperature", ("%.1f C / %.1f F"):format(r1(t), r1(c2f(t))))
    stamped = true
  end
  if h ~= nil then
    setvar("HUMIDITY", r1(h))
    setprop("Humidity", ("%.1f %%"):format(r1(h)))
    stamped = true
  end
  if lux ~= nil then
    setvar("LIGHT_LEVEL", r1(lux))
    setprop("Light Level", ("%.1f lux"):format(r1(lux)))
    stamped = true
    if lux <= DARK_AT and IS_DARK ~= true then
      IS_DARK = true
      setvar("LIGHT_IS_DARK", true)
      pcall(function() C4:FireEvent(1) end)
      dbg(("light -> DARK (%.1f <= %s lux)"):format(r1(lux), tostring(DARK_AT)))
    elseif lux >= BRIGHT_AT and IS_DARK ~= false then
      IS_DARK = false
      setvar("LIGHT_IS_DARK", false)
      pcall(function() C4:FireEvent(2) end)
      dbg(("light -> BRIGHT (%.1f >= %s lux)"):format(r1(lux), tostring(BRIGHT_AT)))
    end
  end
  if stamped then
    local when = os.date("%Y-%m-%d %H:%M:%S")
    setvar("LAST_UPDATED", when); setprop("Last Updated", when)
  end
end
...
function DRV.OnDriverInit()
  local function add(n, t) pcall(function() C4:AddVariable(n, "", t) end) end
  add("TEMPERATURE_C", "FLOAT")
  add("TEMPERATURE_F", "FLOAT")
  add("HUMIDITY", "FLOAT")
  add("LIGHT_LEVEL", "FLOAT")
  add("LIGHT_IS_DARK", "BOOL")
...
function DRV.OnDriverLateInit()
  ...
  read_thresholds()
  dbg("sensor init, aid=" .. tostring(AID)
      .. " dark<=" .. tostring(DARK_AT) .. " bright>=" .. tostring(BRIGHT_AT))
  to_hub("HK_GET_STATE")
end
OPC["Accessory AID"] = function(v)
  AID = aidfmt(v); SENT_T, SENT_H = false, false; to_hub("HK_GET_STATE")
end
OPC["Dark Threshold (lux)"]   = function() read_thresholds(); IS_DARK = nil; reflect() end
OPC["Bright Threshold (lux)"] = function() read_thresholds(); IS_DARK = nil; reflect() end
...
function RFP.RECEIVE_DB(idBinding, tParams)
  ...
  if ok then
    set_iid_maps(map)
    dbg("got DB: Temp iid=" .. tostring(TYPEMAP[HK.TEMP]) .. " Humidity iid=" .. tostring(TYPEMAP[HK.HUM])
        .. " Lux iid=" .. tostring(TYPEMAP[HK.LUX]))
    reflect()
  end
end
