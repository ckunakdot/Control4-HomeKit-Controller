...
local HK = { CUR = "6D", TGT = "7C", STATE_ = "72", HOLD = "6F" }

local function notify(cmd, params) C4:SendToProxy(PROXY, cmd, params, "NOTIFY") end
local function aidfmt(a) local n=tonumber(a); return n and string.format("%.0f", n) or tostring(a) end
local function to_hub(cmd, params) params = params or {}; params.aid = AID; C4:SendToProxy(HUB, cmd, params) end

local function clamp(n) n = tonumber(n) or 0; if n < 0 then return 0 elseif n > 100 then return 100 end return n end

function DRV.OnDriverLateInit()
  AID = aidfmt(Properties and Properties["Accessory AID"] or "1")
  dbg("shade init, aid=" .. tostring(AID))
  to_hub("HK_GET_STATE")
end
OPC["Accessory AID"] = function(v) AID = aidfmt(v); dbg("aid set to " .. tostring(AID)); to_hub("HK_GET_STATE") end

GCPL.OnBindingChanged = function(idBinding, class, bIsBound)
  dbg("binding " .. tostring(idBinding) .. " bound=" .. tostring(bIsBound))
...
local function set_iid_maps(map)
  TYPEMAP = map or {}
  IIDTYPE = {}
  for t, iid in pairs(TYPEMAP) do IIDTYPE[tonumber(iid) or iid] = t end
end
...
function RFP.RECEIVE_DB(idBinding, tParams)
  ...
  if ok then set_iid_maps(map); dbg("got DB: TargetPosition iid=" .. tostring(TYPEMAP[HK.TGT]) ..
      " CurrentPosition iid=" .. tostring(TYPEMAP[HK.CUR])) end
end

local function reflect()
  local pos = STATE[HK.CUR]
  local ps  = STATE[HK.STATE_]
  local mv = 0
  if ps == 0 then mv = 2 elseif ps == 1 then mv = 1 end
  notify("SET_MOVEMENT", { MOVEMENT = mv })
  if (ps == nil or ps == 2) and pos ~= nil then
    notify("STOPPED", { LEVEL = clamp(pos) })
  end
end
...
function RFP.RECEIVE_STATE(idBinding, tParams)
  ...
  if ok and list then apply_chars(list) end
end
...
local function set_target(level)
  local iid = TYPEMAP[HK.TGT]
  if iid then
    dbg("SET target=" .. tostring(clamp(level)) .. " -> iid " .. tostring(iid))
    to_hub("HK_SET_CHAR", { iid = tostring(iid), value = tostring(clamp(level)) })
  else
    dbg("no TargetPosition iid yet (map empty? not bound to hub?)")
  end
end

function RFP.OPEN()  dbg("proxy OPEN");  set_target(100) end
function RFP.UP()    dbg("proxy UP");    set_target(100) end
function RFP.CLOSE() dbg("proxy CLOSE"); set_target(0) end
function RFP.DOWN()  dbg("proxy DOWN");  set_target(0) end

function RFP.STOP()
  dbg("proxy STOP")
  local iid = TYPEMAP[HK.HOLD]
  if iid then to_hub("HK_SET_CHAR", { iid = tostring(iid), value = "true" })
  else dbg("accessory has no HoldPosition; cannot stop mid-travel over HomeKit") end
end

function RFP.TOGGLE()
  local cur = tonumber(STATE[HK.CUR]) or 0
  dbg("proxy TOGGLE (cur=" .. cur .. ")")
  set_target(cur >= 50 and 0 or 100)
end

function RFP.SET_LEVEL(idBinding, tParams)
  local lvl = tParams.LEVEL or tParams.TARGET_LEVEL or tParams.TARGET or tParams.LEVEL_TARGET
  dbg("proxy SET_LEVEL " .. tostring(lvl))
  set_target(lvl)
end
RFP.SET_LEVEL_TARGET = RFP.SET_LEVEL
