...
do
	...
	HK_AID      = HK_AID or "1"
	HK_ZONE     = HK_ZONE or "Auto"
	HK_SERVICES = HK_SERVICES or {}
	HK_FLAT     = HK_FLAT or {}
	HK_IID      = HK_IID or {}
	HKC = { ACTIVE="B0", INUSE="D2", SETDUR="D3", REMAIN="D4", VTYPE="D5", NAME="23", SVC_VALVE="D0" }
	HK_CMD_AT   = HK_CMD_AT or 0
	HK_GUARD    = 12
	SYS_AID        = SYS_AID or "1"
	SYS_ACTIVE_IID = SYS_ACTIVE_IID or nil
end
...
function hk_to_hub_aid (cmd, aid, p)
	p = p or {}
	p.aid = tostring (aid)
	C4:SendToProxy (HK_HUB, cmd, p)
end
...
function hk_resolve ()
	HK_IID = {}
	local chars
	if (HK_ZONE and HK_ZONE ~= 'Auto' and HK_ZONE ~= '') then
		for _, sv in ipairs (HK_SERVICES) do
			if (sv.name == HK_ZONE and sv.chars and sv.chars [HKC.ACTIVE]) then
				chars = sv.chars ; break
			end
		end
	end
	if (not chars) then
		for _, sv in ipairs (HK_SERVICES) do
			if (sv.chars and sv.chars [HKC.ACTIVE]) then chars = sv.chars ; break end
		end
		chars = chars or HK_FLAT
	end
	HK_IID.active = chars [HKC.ACTIVE]
	HK_IID.dur    = chars [HKC.SETDUR]
	HK_IID.inuse  = chars [HKC.INUSE]
	HK_IID.remain = chars [HKC.REMAIN]
	dbg ('HK zone "' .. tostring(HK_ZONE) .. '" -> active iid ' .. tostring(HK_IID.active)
		.. ' dur iid ' .. tostring(HK_IID.dur))
end

function DEVICE.HKSet (bOn)
	if (not HOMEKIT) then return end
	if (not HK_IID.active) then dbg ('HKSet: no valve iid resolved yet') return end
	HK_CMD_AT = os.time ()
	dbg ('HKSet ' .. tostring(bOn) .. ' -> active iid ' .. tostring(HK_IID.active))
	if (bOn) then
		local secs = 0
		if (gCurrentState ~= 'On' and gCurrentState ~= 'Off') then
			secs = (tonumber (string.match (gCurrentState, '(%d+)')) or 0) * 60
		end
		if (SYS_AID and SYS_AID ~= '' and SYS_AID ~= '0' and SYS_ACTIVE_IID) then
			dbg ('activating system aid ' .. SYS_AID .. ' active iid ' .. tostring(SYS_ACTIVE_IID))
			hk_to_hub_aid ('HK_SET_CHAR', SYS_AID, { iid = tostring(SYS_ACTIVE_IID), value = '1' })
		elseif (SYS_AID and SYS_AID ~= '' and SYS_AID ~= '0') then
			dbg ('WARNING: system active iid not resolved yet; re-requesting system DB')
			hk_to_hub_aid ('HK_GET_STATE', SYS_AID)
		end
		if (secs > 0 and HK_IID.dur) then
			local data = '[{"iid":"' .. tostring(HK_IID.dur) .. '","value":"' .. tostring(secs) .. '"},'
			          .. '{"iid":"' .. tostring(HK_IID.active) .. '","value":"1"}]'
			dbg ('HKSet atomic: dur ' .. secs .. 's + active -> ' .. data)
			hk_to_hub ('HK_SET_CHARS', { data = data })
		else
			hk_to_hub ('HK_SET_CHAR', { iid = tostring(HK_IID.active), value = '1' })
		end
	else
		hk_to_hub ('HK_SET_CHAR', { iid = tostring(HK_IID.active), value = '0' })
	end
end

function DEVICE.HKReflect (t, v)
	if (t == HKC.REMAIN) then
		local n = tonumber (v) ; if (n) then C4:SetVariable ('COUNTDOWN', math.floor (n / 60)) end
		return
	end
	if (t == HKC.ACTIVE or t == HKC.INUSE) then
		dbg ('valve ' .. t .. ' reported = ' .. tostring(v))
		C4:SetVariable ('STATE', (tonumber (v) == 1) and 1 or 0)
	end
end
...
function ON_PROPERTY_CHANGED.ControlMethod (value)
	...
	if (value == 'Relay') then
		...
		C4:SetPropertyAttribs('Zone', 1)
		C4:SetPropertyAttribs('Irrigation System AID', 1)
	...
	elseif (value == 'HomeKit') then
		...
		C4:SetPropertyAttribs('Zone', 0)
		C4:SetPropertyAttribs('Irrigation System AID', 0)
		hk_to_hub ('HK_GET_STATE')
		if (SYS_AID and SYS_AID ~= '' and SYS_AID ~= '0') then hk_to_hub_aid ('HK_GET_STATE', SYS_AID) end
...
function ON_PROPERTY_CHANGED.Zone (value)
	HK_ZONE = tostring (value or 'Auto')
	hk_resolve ()
end

function ON_PROPERTY_CHANGED.IrrigationSystemAID (value)
	SYS_AID = tostring (value or '')
	SYS_ACTIVE_IID = nil
	if (HOMEKIT and SYS_AID ~= '' and SYS_AID ~= '0') then
		hk_to_hub_aid ('HK_GET_STATE', SYS_AID)
	end
end
...
function PROXY_CMDS.RECEIVE_SERVICES (tParams)
	...
	for _, sv in ipairs (HK_SERVICES) do
		if (sv.name and sv.chars and sv.chars [HKC.ACTIVE] and not seen [sv.name]) then
			seen [sv.name] = true ; names [#names + 1] = (sv.name:gsub (',', ' '))
		end
	end
	C4:UpdatePropertyList ('Zone', table.concat (names, ','))
	hk_resolve ()
end

function PROXY_CMDS.RECEIVE_DB (tParams)
	local ok, map = pcall (function () return C4:JsonDecode (tParams.data) end)
	if (not (ok and type(map) == 'table')) then return end
	if (hk_same_aid (tParams.aid)) then
		HK_FLAT = map ; hk_resolve ()
	elseif (SYS_AID and SYS_AID ~= '' and tonumber(tParams.aid) == tonumber(SYS_AID)) then
		SYS_ACTIVE_IID = map [HKC.ACTIVE]
		dbg ('system aid ' .. tostring(tParams.aid) .. ' active iid = ' .. tostring(SYS_ACTIVE_IID))
	end
end
...
function PROXY_CMDS.RECEIVE_STATE (tParams)
	...
	local ok, list = pcall (function () return C4:JsonDecode (tParams.data) end)
	if (not (ok and type(list) == 'table')) then return end
	local want = { [tostring(HK_IID.active)] = HKC.ACTIVE,
	               [tostring(HK_IID.inuse)]  = HKC.INUSE,
	               [tostring(HK_IID.remain)] = HKC.REMAIN }
	for _, c in ipairs (list) do
		local t = want [tostring(c.iid)]
		if (t) then DEVICE.HKReflect (t, c.value) end
	end
end
...
function OnBindingChanged (idBinding, strClass, bIsBound)
	if (tonumber(idBinding) == HK_HUB and bIsBound) then
		...
		hk_to_hub ('HK_GET_STATE')
		if (SYS_AID and SYS_AID ~= '' and SYS_AID ~= '0') then
			hk_to_hub_aid ('HK_GET_STATE', SYS_AID)
		end
	end
end
